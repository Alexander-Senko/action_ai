...
== Structured output (data schemas)

Action AI can instruct the model to return structured JSON that maps directly
to an ActiveModel model. Two features cooperate to make this work:

=== +.schema+ on ActiveModel models

Any model that includes +ActiveModel::API+ automatically gains a +.schema+
class method that returns a +Schematist::Schema+ instance derived from the model's
attribute types.
...
* +:date+, +:datetime+, +:time+ → JSON +string+ with +format+ set to
  +"date"+, +"date-time"+, or +"time"+ respectively
* +:binary+ → JSON +string+ with +contentEncoding+ set to +"base64"+

Other types are not included in the schema.

=== +returns+ in agent actions

Call +returns ModelClass+ inside an action method to declare the expected
return type. Action AI will:

1. Configure the chat to request output matching the model's JSON schema
   (+chat.with_schema ModelClass.schema+).
